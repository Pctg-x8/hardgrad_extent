...
vec2 search_diag1(vec2 uv, vec2 dir, out vec2 edge)
{
	...
	vec3 t = vec3(rtdesc.metrics.xy, 1.0f);
	while(coord.z < float(SMAA_MAX_SEARCH_STEPS_DIAG - 1) && coord.w > 0.9)
	{
		coord.xyz = mad(t, vec3(dir, 1.0f), coord.xyz);
		edge = textureLod(intex[0], coord.xy, 0.0f).rg;
		coord.w = dot(edge, vec2(0.5, 0.5));
	}
	return coord.zw;
}
...
vec2 area_diag(vec2 dist, vec2 e, float offset)
{
	...
	return areatex_select(textureLod(intex[1], texcoord, 0.0f));
}
vec2 calculate_diag_weights(vec2 texcoord, vec2 edge)
{
	vec2 weights = vec2(0.0f, 0.0f);
	...
	else d.xz = vec2(0.0f, 0.0f);
	d.yw = search_diag1(texcoord, vec2(1.0f, -1.0f), end);

	if(d.x + d.y > 2.0f)
	{
		// Fetch the crossing edges
		vec4 coords = mad(vec4(-d.x + 0.25, d.x, d.y, -d.y - 0.25f), rtdesc.metrics.xyxy, texcoord.xyxy);
		vec4 c;
		c.xy = textureLodOffset(intex[0], coords.xy, 0.0f, ivec2(-1, 0)).rg;
		c.zw = textureLodOffset(intex[0], coords.zw, 0.0f, ivec2(1, 0)).rg;
		c.yxwz = decode_diag_bilinear_access(c.xyzw);

		// Merge crossing edges at each side into a single value
		vec2 cc = mad(vec2(2.0f, 2.0f), c.xz, c.yw);
		// Remove the crossing edge if we didn't found the end of the line
		movc(bvec2(step(0.9f, d.zw)), cc, vec2(0.0f, 0.0f));
		// Fetch the areas for this line
		weights += area_diag(d.xy, cc, 0.0f);
	}

	// Search for the line ends
	d.xz = search_diag2(texcoord, vec2(-1.0f, -1.0f), end);
	if(textureLodOffset(intex[0], texcoord, 0.0f, ivec2(1, 0)).r > 0.0f)
	{
		d.yw = search_diag2(texcoord, vec2(1.0f, 1.0f), end);
		d.y += float(end.y > 0.9);
	}
	else d.yw = vec2(0.0f, 0.0f);

	if(d.x + d.y > 2.0f)
	{
		// Fetch the crossing edges
		vec4 coords = mad(vec4(-d.x, -d.x, d.y, d.y), rtdesc.metrics.xyxy, texcoord.xyxy);
		vec4 c;
		c.x = textureLodOffset(intex[0], coords.xy, 0.0f, ivec2(-1, 0)).g;
		c.y = textureLodOffset(intex[0], coords.xy, 0.0f, ivec2(0, -1)).r;
		c.zw = textureLodOffset(intex[0], coords.zw, 0.0f, ivec2(1, 0)).gr;
		vec2 cc = mad(vec2(2.0f, 2.0f), c.xz, c.yw);
...
float search_length(vec2 edge, float offset)
{
	...
	return searchtex_select(textureLod(intex[2], mad(scale, edge, bias), 0.0f));
}
float search_x_left(vec2 texcoord, float end)
{
	...
	{
		e = textureLod(intex[0], texcoord, 0.0f).rg;
		texcoord = mad(-vec2(2.0f, 0.0f), rtdesc.metrics.xy, texcoord);
	}

	float offset = mad(-(255.0f / 127.0f), search_length(e, 0.0f), 3.25f);
	return mad(rtdesc.metrics.x, offset, texcoord.x);
}
...
float search_y_up(vec2 texcoord, float end)
{
	...
	{
		e = textureLod(intex[0], texcoord, 0.0f).rg;
		texcoord = mad(-vec2(0.0f, 2.0f), rtdesc.metrics.xy, texcoord);
	}
	float offset = mad(-(255.0f / 127.0f), search_length(e.gr, 0.0f), 3.25f);
	return mad(rtdesc.metrics.y, offset, texcoord.y);
}
...
void detect_horizontal_corner_pattern(inout vec2 weights, vec4 texcoord, vec2 d)
{
	...
	factor.x -= rounding.x * textureLodOffset(intex[0], texcoord.xy, 0.0f, ivec2(0, 1)).r;
	factor.x -= rounding.y * textureLodOffset(intex[0], texcoord.zw, 0.0f, ivec2(1, 1)).r;
	factor.y -= rounding.x * textureLodOffset(intex[0], texcoord.xy, 0.0f, ivec2(0, -2)).r;
	factor.y -= rounding.y * textureLodOffset(intex[0], texcoord.zw, 0.0f, ivec2(1, -2)).r;
...
void detect_vertical_corner_pattern(inout vec2 weights, vec4 texcoord, vec2 d)
{
	...
	vec2 factor = vec2(1.0f, 1.0f);
	factor.x -= rounding.x * textureLodOffset(intex[0], texcoord.xy, 0.0f, ivec2(1, 0)).g;
	factor.x -= rounding.y * textureLodOffset(intex[0], texcoord.zw, 0.0f, ivec2(1, 1)).g;
	factor.y -= rounding.x * textureLodOffset(intex[0], texcoord.xy, 0.0f, ivec2(-2, 0)).g;
	factor.y -= rounding.y * textureLodOffset(intex[0], texcoord.zw, 0.0f, ivec2(-2, 1)).g;
...
void main()
{
	...
	vec2 e = texture(intex[0], uv_pixcoord.xy).rg;
	...
	if(e.g > 0.0f)
	{
		...
		if(weights.r == -weights.g)
		{
			...
			float e1 = textureLod(intex[0], coords.xy, 0.0f).r;

			coords.z = search_x_right(offsets[0].zw, offsets[2].y);
			d.y = coords.z;

			d = abs(round(mad(rtdesc.metrics.zz, d, -uv_pixcoord.zz)));
			vec2 sqrt_d = sqrt(d);
			float e2 = textureLodOffset(intex[0], coords.zy, 0.0f, ivec2(1, 0)).r;
	...
	if(e.r > 0.0f)
	{
		...
		float e1 = textureLod(intex[0], coords.xy, 0.0f).g;
		coords.z = search_y_down(offsets[1].zw, offsets[2].w);
		d.y = coords.z;

		d = abs(round(mad(rtdesc.metrics.ww, d, -uv_pixcoord.ww)));
		vec2 sqrt_d = sqrt(d);

		float e2 = textureLodOffset(intex[0], coords.xz, 0.0f, ivec2(0, 1)).g;
