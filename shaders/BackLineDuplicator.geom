...
layout(location = 0) in uint index_mult[];
...
void main()
{
    if(index_mult[0] > 0)
    {
        vec4 offset_layers = offset[index_mult[0] - 1];
        if(offset_layers.w > gl_InvocationID)
        {
            vec4 offsetter = vec4(0.0f, 0.0f, gl_InvocationID, 0.0f) + vec4(offset_layers.xyz, 0.0f);
            gl_Position = (gl_in[0].gl_Position + offsetter) * persp_projection_matrix; color = vec4(r, g, b, a); EmitVertex();
            gl_Position = (gl_in[1].gl_Position + offsetter) * persp_projection_matrix; color = vec4(r, g, b, a); EmitVertex();
            gl_Position = (gl_in[2].gl_Position + offsetter) * persp_projection_matrix; color = vec4(r, g, b, a); EmitVertex();
            gl_Position = (gl_in[3].gl_Position + offsetter) * persp_projection_matrix; color = vec4(r, g, b, a); EmitVertex();
            gl_Position = (gl_in[0].gl_Position + offsetter) * persp_projection_matrix; color = vec4(r, g, b, a); EmitVertex();
            EndPrimitive();
        }
    }
